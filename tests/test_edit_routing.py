...
def test_edit_mode_pro_routes_to_selected_service_for_path(monkeypatch, tmp_path):
    from gpt_image_2_polza_mcp_server.config.settings import ModelTier
    ...
    monkeypatch.setattr("gpt_image_2_polza_mcp_server.services.get_model_selector", lambda: model_selector)
    monkeypatch.setattr(
        "gpt_image_2_polza_mcp_server.tools.generate_image._get_enhanced_image_service",
        lambda: enhanced,
    )
...
def test_edit_mode_nb2_routes_to_selected_service_for_file_id(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("gpt_image_2_polza_mcp_server.services.get_model_selector", lambda: model_selector)
    monkeypatch.setattr("gpt_image_2_polza_mcp_server.services.get_files_api_service", lambda: files_api)
    monkeypatch.setattr(
        "gpt_image_2_polza_mcp_server.tools.generate_image._get_enhanced_image_service",
        lambda: enhanced,
    )
...
@pytest.mark.unit
def test_generate_mode_gpt_image_2_routes_without_gemini_only_options(monkeypatch, tmp_path):
    from gpt_image_2_polza_mcp_server.config.settings import ModelTier

    gen_fn = _get_generate_image_fn()

    gpt_service = _FakeProLikeService()
    enhanced = _FakeEnhancedImageService()
    model_selector = _FakeModelSelector(
        selected_service=gpt_service,
        selected_tier=ModelTier.GPT_IMAGE_2,
        model_id="openai/gpt-5.4-image-2",
        name="GPT Image 2",
    )

    monkeypatch.setattr(
        "gpt_image_2_polza_mcp_server.services.get_model_selector",
        lambda: model_selector,
    )
    monkeypatch.setattr(
        "gpt_image_2_polza_mcp_server.tools.generate_image._get_enhanced_image_service",
        lambda: enhanced,
    ...
    result = gen_fn(
        ...
        mode="generate",
        model_tier="gpt-image-2",
        resolution="4k",
        aspect_ratio="16:9",
        output_path=str(tmp_path),
    )

    assert gpt_service.calls
    call = gpt_service.calls[0][1]
    assert call["resolution"] == "4k"
    assert call["aspect_ratio"] == "16:9"
    assert "thinking_level" not in call
    assert "enable_grounding" not in call
    assert enhanced.calls == []
    assert result.structured_content["model_tier"] == "gpt-image-2"
    assert result.structured_content["model_id"] == "openai/gpt-5.4-image-2"
