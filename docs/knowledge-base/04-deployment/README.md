...
python -m gpt_image_2_polza_mcp_server.server
```
...
./scripts/cleanup-ports.sh && fastmcp dev gpt_image_2_polza_mcp_server.server:create_app

# HTTP transport
FASTMCP_TRANSPORT=http python -m gpt_image_2_polza_mcp_server.server
```
...
```bash
export GPT_IMAGE_AUTH_METHOD=vertex_ai
