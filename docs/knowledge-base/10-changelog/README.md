...
- `Gemini31ImageConfig`: corrected `supports_thinking=True` (was `False`) and added `supports_extreme_aspect_ratios=True`
...
- Added `ModelTier.NB2` enum and `Gemini31ImageConfig(ProImageConfig)` — inherits 4K + grounding, overrides model name + timeout, enables thinking (`supports_thinking=True`) and extreme aspect ratios (`4:1`, `1:4`, `8:1`, `1:8`)
