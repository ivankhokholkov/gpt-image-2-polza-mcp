...
File: `gpt_image_2_polza_mcp_server/services/model_selector.py`
...
**NB2 reuses `ProImageService`** — `Gemini31ImageConfig` extends `ProImageConfig` with `supports_thinking=True`, `supports_extreme_aspect_ratios=True`, and model name `gemini-3.1-flash-image-preview`. `isinstance(config, ProImageConfig)` stays `True`, so `GeminiClient._filter_parameters()` works unchanged.

## Authentication Strategy

File: `gpt_image_2_polza_mcp_server/config/settings.py` + `services/gemini_client.py`
