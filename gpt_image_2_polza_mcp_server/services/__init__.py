...
_gpt_image_2_gemini_client: GeminiClient | None = None
_gpt_image_2_service: ProImageService | None = None
...
def initialize_services(server_config: ServerConfig, gemini_config: GeminiConfig):
    ...
    global \
        ...
        _nb2_gemini_client, \
        _nb2_image_service, \
        _gpt_image_2_gemini_client, \
        _gpt_image_2_service, \
        _model_selector, \
        _server_config
    ...
    nb2_config = Gemini31ImageConfig()
    ...
    # Create GPT Image 2 service (OpenAI via Polza)
    gpt_image_2_config = GptImage2Config()
    _gpt_image_2_gemini_client = GeminiClient(server_config, gpt_image_2_config)
    _gpt_image_2_service = ProImageService(
        _gpt_image_2_gemini_client, gpt_image_2_config, _image_storage_service
    )
    ...
    _model_selector = ModelSelector(
        ...
        _gpt_image_2_service,  # GPT Image 2 service
        selection_config,
    )
...
def get_nb2_image_service() -> ProImageService:
    """Get the legacy Gemini 3.1 image service instance."""
    if _nb2_image_service is None:
        raise RuntimeError("Services not initialized. Call initialize_services() first.")
    return _nb2_image_service


def get_gpt_image_2_service() -> ProImageService:
    """Get the GPT Image 2 service instance."""
    if _gpt_image_2_service is None:
        raise RuntimeError("Services not initialized. Call initialize_services() first.")
    return _gpt_image_2_service
